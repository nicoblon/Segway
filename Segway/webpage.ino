...
// Set hotspot name and password here
...
const char index_html[] PROGMEM = R"rawliteral(
  ...
  <html>
      ...
      <div class="container-flex">
        ...
        <div class="column">
          ...
          <div class="section">
            ...
            <p><span id="textDistanceAdjustmentFactorVal">Distance Adj. (current: %distanceAdjustmentFactor%) </span>
            <input type="number" id="DisAd" value="%distanceAdjustmentFactor%" min="0" max="1" step="0.01"> 
            <button onclick="implement_distanceAdjustmentFactor()">Submit</button></p>
          </div>
        </div>
    ...
    <script>
      ...
      function implement_angleAdjustmentFactor(){
          var angleAdjustmentFactor_val = document.getElementById("AngAd").value;
          document.getElementById("textLeftMotorAdjustmentVal").innerHTML = "Angle Adj. (current: " + angleAdjustmentFactor_val + ") ";
          console.log(angleAdjustmentFactor_val);
          var xhr = new XMLHttpRequest();
          xhr.open("GET", "/angleadjustmentfactor?AngAd="+angleAdjustmentFactor_val, true);
          xhr.send();
      }

      function implement_distanceAdjustmentFactor(){
          var distanceAdjustmentFactor_val = document.getElementById("DisAd").value;
          document.getElementById("textRightMotorAdjustmentVal").innerHTML = "Distance Adj. (current: " + distanceAdjustmentFactor_val + ") ";
          console.log(distanceAdjustmentFactor_val);
          var xhr = new XMLHttpRequest();
          xhr.open("GET", "/distanceadjustmentfactor?DisAd="+distanceAdjustmentFactor_val, true);
          xhr.send();
      }
...
String processor(const String& var){
  if (var == "PPS"){
    ...
    return x_ref_val;
  }else if (var == "angleAdjustmentFactor"){
    return angleAdjustmentFactor_val;
  }else if (var == "distanceAdjustmentFactor"){
    return distanceAdjustmentFactor_val;
  }else if (var == "D_start"){
    return D_start_val;
...
// PID values (text)
  ...
  String angleAdjustmentFactor_val = String(angleAdjustmentFactor);
  String distanceAdjustmentFactor_val = String(distanceAdjustmentFactor);
...
void serverStuff(void){
  ...
  server.on("/angleadjustmentfactor", HTTP_GET, [] (AsyncWebServerRequest *request) {
    String inputMessage;
    // GET input1 value on <ESP_IP>/proportional?KP=<inputMessage>
    if (request->hasParam(AngAd_input)) {
      inputMessage = request->getParam(AngAd_input)->value();
      angleAdjustmentFactor_val = inputMessage;
      angleAdjustmentFactor = angleAdjustmentFactor_val.toFloat();
    }
    else {
      inputMessage = "No message sent";
    }
  ...
  server.on("/distanceadjustmentfactor", HTTP_GET, [] (AsyncWebServerRequest *request) {
    String inputMessage;
    // GET input1 value on <ESP_IP>/proportional?KP=<inputMessage>
    if (request->hasParam(DisAd_input)) {
      inputMessage = request->getParam(DisAd_input)->value();
      distanceAdjustmentFactor_val = inputMessage;
      distanceAdjustmentFactor = distanceAdjustmentFactor_val.toFloat();
    }
    else {
      inputMessage = "No message sent";
    }
  ...
  // request live values to show on the website -> be careful as this can make the Segway buggy!
  // Try not to use too many at once!
    // and if possible use none
  ...
  server.on("/cmmd", HTTP_GET, [] (AsyncWebServerRequest *request) {
    ...
    if (request->hasParam("cmd")) {
      ...
      if (inputMessage == "W") {
        x_ref += 50; // Positon ref if now +50 cm if we press Forward 
        start = true;
      }
      else if (inputMessage == "S") {
        x_ref -= 50; // Positon ref if now -50 cm if we press Backward
        start = true;
      }
      else if(inputMessage == "X") {
        reset = true; // Reset all quantities related to segway position to 0 and stop the segway
      }
        ...
        circuit = false;  // sets the right parameters for the Segway, whether following a path or racing on the circuit
        settingsPath1 = true;
        settingsPath2 = false;
        settingsPath3 = false;
      }
